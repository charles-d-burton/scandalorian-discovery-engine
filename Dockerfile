...
WORKDIR /app
COPY ./ .
RUN GOOS=linux GOARCH=amd64 go build -a -installsuffix cgo -ldflags="-w -s" -o discovery-engine
...
COPY --from=build-amd64 /app/discovery-engine /go/bin/discovery-engine
ENTRYPOINT ["/go/bin/discovery-engine"]
